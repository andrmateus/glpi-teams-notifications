...
from db_glpi import get_expired_tickets, get_expiring_tickets
...
sla_expiring_log_file_path = 'sla_expiring_notifications.log'
...
if not os.path.exists(sla_expiring_log_file_path):
    with open(sla_expiring_log_file_path, 'w') as f:
        f.write('id,fila_atendimento,data_notificacao\n')
...
def send_sla_expiring_notifications():
    # Leia as notificações existentes do arquivo de log
    notified_tickets = set()
    with open(sla_expiring_log_file_path, 'r') as f:
        next(f)  # Pula a primeira linha (cabeçalho)
        for line in f:
            ticket_id, fila_atendimento, data_notificacao = line.strip().split(',')
            notified_tickets.add(int(ticket_id))
    ...
    # Obtenha a lista de chamados com SLA expirado
    expiring_tickets = get_expiring_tickets()

    # Filtra os chamados que já foram notificados
    new_tickets = expiring_tickets[~expiring_tickets['id'].isin(notified_tickets)]

    # Envia uma notificação para cada novo chamado e salva no arquivo de log
    with open(sla_expiring_log_file_path, 'a') as f:
        for _, ticket in new_tickets.iterrows():
...
def main():
    ...
    send_sla_expiring_notifications()
